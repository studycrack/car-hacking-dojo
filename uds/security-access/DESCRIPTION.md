...
*  Reflashing, running actuator tests, and disarming an immobiliser all sit behind service `0x27`.
*  **SecurityAccess** is a challenge-response exchange.
   1.  `10 03` puts you in an extended session.
   2.  `27 01` requests a seed. The controller answers `67 01` and four random bytes.
   3.  You compute a key from that seed and send `27 02 <key>`. This controller uses a **sixteen-bit key**.
   4.  If the key is right the controller answers `67 02`.
...
*  But three wrong keys and the controller answers `7F 27 36` (exceedNumberOfAttempts) and refuses security requests for ten seconds. At that rate an exhaustive search takes days.
...
*  Work that into the loop and sweep the whole sixteen bits.
   *  `7F 27 36` is the third wrong key. While the lockout lasts you get `7F 27 37` instead.

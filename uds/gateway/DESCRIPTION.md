...
*  Sweep its DIDs and read the routing table. Script the sweep with `isotp.py` and `vcan.py` from `/challenge`.
...
bus = vcan.Bus("vcan0")
gateway = 0x000                        # the address you just found
response = isotp.request(bus, gateway, gateway + 8,
                         bytes.fromhex("22F18C"), timeout=0.2)
```

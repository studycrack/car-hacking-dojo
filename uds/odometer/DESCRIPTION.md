...
*  Service `0x2E`, **WriteDataByIdentifier**, is the other half of the `0x22` you already know. Same identifier space, but reading is served in the default session and writing is not.
...
*  Except the cluster is no longer the only thing that remembers. The odometer is recorded across more than one controller precisely so that changing one is detectable.
...
*  Enter an extended session on each of them. The session is per controller, not per bus.
